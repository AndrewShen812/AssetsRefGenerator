...
//        System.out.println("assets/images/3.0x/doge-1.jpeg".matches(PATTERN));
        System.out.println("'assets/images/3.0x/ic_default_avatar'".matches("^[\"']?(asset(s)?(/([-\\w]+|[1-9]\\.\\dx))*/)?[-\\w]+(.(jp(e)?g|png|webp|bmp))?[\"']?$"));
        System.out.println("'ic_default_avatar'".matches("^[\"']?(asset(s)?(/([-\\w]+|[1-9]\\.\\dx))*/)?[-\\w]+(.(jp(e)?g|png|webp|bmp))?[\"']?$"));
    }
}
